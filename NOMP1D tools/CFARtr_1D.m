function [T_judgement, Threshold_CUT] = CFARtr_1D(y, sampledManifold, ...
    training_cells, alpha_set)
...
prob_ind_ext = repmat(prob_ind, [3, 1]);
% guardidx_set = (peak_idx - guard_cells) : (peak_idx + guard_cells);
training_n = floor(training_cells / 2) + guard_cells;
...
sigma_hat = (sum(Y_training_win) - sum(Y_guard_win)) / training_cells;
